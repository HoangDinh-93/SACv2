...
const email = "dinhtienhoang540@gmail.com";
...
test("Get URLs", async ({ gmailPage }) => {
  while (true) {
    // const filePath = path.join(__dirname, "../data/cuongdog50.json");
    ...
    const result =
      ...
      (match ? match[1] : "");
    const urls = readJsonFile<string[]>("data/cuongdog50.json");
    urls.push(result);
    // fs.writeFileSync(filePath, JSON.stringify(urls, null, 2), "utf-8");
    writeJsonFile("data/cuongdog50.json", urls);
